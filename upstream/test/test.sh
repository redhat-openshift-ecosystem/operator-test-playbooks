...
POD_START_RETRIES_LONG_DEPLOYMENT_WAIT_RETRIES=2250
...
function ExecParameters() {
    ...
    # Handle index_check
    [[ $1 == orange* ]] &&[ "$OP_TEST_STREAM" = "community-operators" ] && OP_TEST_EXEC_USER_INDEX_CHECK="-e run_prepare_catalog_repo_upstream=true -e stream_kind=openshift_upstream -e bundle_index_image=quay.io/openshift-community-operators/catalog:latest -e operator_base_dir=$OP_TEST_BASE_DIR/$OP_TEST_STREAM"
    ...
    [[ $1 == orange* ]] && [ "$OP_TEST_STREAM" = "community-operators" ] && OP_TEST_EXEC_USER_INDEX_CHECK="$OP_TEST_EXEC_USER_INDEX_CHECK -e stream_kind=openshift_upstream"
...
for t in $TESTS;do
    ...
    [[ $OP_TEST_IIB_INSTALL -eq 1 ]] && [[ $IIB_INSTALLED -eq 0 ]] && iib_install && IIB_INSTALLED=1
